...
// Composant Tooltip personnalisé Robuste
const CustomRadarTooltip = ({ active, payload }: any) => {
  if (!active || !payload || payload.length === 0) return null;

  // On cherche la donnée du radar d'apport (pct). 
  // Si Recharts ne l'envoie pas directement, on prend le premier élément qui a les données.
  const data = payload.find((p: any) => p.dataKey === "pct") || payload[0];
  
  if (!data || !data.payload || !data.payload.fullName) return null;

  const item = data.payload;

  return (
    <div className="bg-popover border border-border rounded-xl p-3 shadow-md text-[12px] min-w-[160px] space-y-1 animate-in fade-in zoom-in-95">
      <p className="font-bold text-foreground mb-1 border-b pb-1 border-border">
        {item.fullName}
      </p>
